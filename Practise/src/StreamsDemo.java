...
public class StreamsDemo {
	...
	public static void main(String[] args) {
		...
		System.out.println(
				"Sorted Names -> " + customers.stream().map(Customer::getName).sorted().collect(Collectors.toList()));
		...
		System.out.println("Sorted based on age using new form -> "
				+ customers.stream().sorted(Comparator.comparing(Customer::getAge)).collect(Collectors.toList()));
		...
		System.out.println("Names of customers except youngest two ->"
				+ customers.stream().sorted(Comparator.comparing(Customer::getAge)).skip(2).map(cust -> cust.getName())
						.collect(Collectors.toList()));

		System.out
				.println("Sum of first 4 elements in List -> " + integerList.stream().limit(4).reduce(0, Integer::sum));

		// findFirst() and findAny()
		Optional<Customer> findingCustomer = customers.stream().filter(c -> c.getAccNumber() < 12350d).findFirst();
		if (findingCustomer.isPresent())
			System.out.println("Customer using findAny() -> " + findingCustomer.get());
		else
